...
export function setCorsHeaders(
  ...
  const allowedOrigin = requestOrigin && CORS_ALLOWED_ORIGINS.includes(requestOrigin)
    ? requestOrigin
...
export function createApiResponse<T>(
  ...
  const isApiResponseShape = (obj: any): obj is { status: number; message: string; payload: any } => {
    return obj &&
           typeof obj === 'object' &&
           !Array.isArray(obj) &&
           'status' in obj &&
  ...
  return setCorsHeaders(response, undefined, request);
}
