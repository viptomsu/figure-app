...
const HF_API_KEY = process.env.REACT_APP_HF_API_KEY; // Make sure to replace with your actual API key
export const checkProfanityWithHuggingFace = async (
  text: string
): Promise<boolean> => {
  try {
    ...
    const toxicScore =
      response.data.scores[response.data.labels.indexOf("toxic")];
