...
const OrderDetailsModal = NiceModal.create<{ order: any }>(({ order }) => {
  ...
  return (
    <Dialog open={modal.visible} onOpenChange={(open) => { if (!open) { modal.hide(); setTimeout(() => modal.remove(), 300); } }}>
      <DialogContent className="max-w-4xl">
        ...
        <div className="border border-gray-200 rounded-lg p-4 mt-4">
          ...
          <div className="flex justify-between mb-2">
            ...
            <p>
              {formatCurrency(
                order.totalPrice / (1 - order.discount / 100)
              )}
            </p>
          </div>
          <div className="flex justify-between mb-2">
          ...
          <div>
            <div>
              <p>
                ...
                {order.addressBook.recipientName}
              </p>
              <p>
            ...
            <div>
              <p>
                ...
                {order.addressBook.phoneNumber ||
                  "Không có số điện thoại"}
              </p>
            </div>
            <div>
              <p>
                <strong>Địa chỉ:</strong> {order.addressBook.address},{" "}
                {order.addressBook.ward},{" "}
                {order.addressBook.district},{" "}
                {order.addressBook.city}
              </p>
            </div>
          </div>
        </div>
